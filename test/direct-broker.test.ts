...
test("durable full-permissions config auto-accepts first-party app access without a model or UI vote", async () => {
	const root = await mkdtemp(path.join(os.tmpdir(), "direct-broker-full-elicit-test."));
	try {
		const { script, log } = await makeFake(root);
		...
		assert.equal(accepted.approvalRequests, 1);
		const records = (await readFile(log, "utf8")).trim().split("\n").map((line) => JSON.parse(line));
		assert.deepEqual(records.find((item) => item.method === "initialize")?.params?.capabilities, { mcpServerOpenaiFormElicitation: true });
		assert.equal(records.find((item) => item.method === "thread/start")?.params?.approvalPolicy, "on-request");
		assert.deepEqual(records.find((item) => item.id === "approval-1" && item.result)?.result, {
			action: "accept",
			content: {},
...
test("durable safe config declines first-party app access without a model or UI vote", async () => {
	const root = await mkdtemp(path.join(os.tmpdir(), "direct-broker-safe-elicit-test."));
	try {
		const { script, log } = await makeFake(root);
