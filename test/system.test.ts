...
test("app identity preserves the official bundle ID while canonicalizing leases", () => {
	...
	assert.equal(resolveAppIdentity("com.apple.calculator").leaseId, "com.apple.calculator");
	assert.equal(resolveAppIdentity("Calculator").leaseId, "com.apple.calculator");
	assert.equal(resolveAppIdentity("Unregistered App").leaseId, "name:unregistered app");
	assert.deepEqual(resolveAppIdentity("com.example.UninstalledApp"), {
		leaseId: "com.example.uninstalledapp",
	});
});
